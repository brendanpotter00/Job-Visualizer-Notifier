...
Re-parented 2026-09-09: originally chained off ``cfa099f2e1e0`` (the Wave-1
factory-seed merge head). When feat/perf-wave2 was brought up to date with main,
main had advanced ~23 commits including a long run of company-seed migrations;
rather than add a merge migration this revision was re-pointed onto main's then
current single head ``87fe6224b0b5`` so the chain stays LINEAR and single-headed
with this revision last. The body is independent of the seed migrations (it only
touches ``job_listings``), so re-parenting is a no-op for the schema it produces.
