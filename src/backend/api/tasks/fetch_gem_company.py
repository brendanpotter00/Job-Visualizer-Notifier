"""Procrastinate task: fetch a single Gem company's job board.

Per-company unit of work for the gem fan-out cron (see Unit 5).
Fetches the live Gem Job Board API for one company, upserts rows
...
from ..services.gem_client import SOURCE_ID, fetch_jobs, transform_to_job_listings
...
async def fetch_gem_company(
    company_id: str,
    board_token: str,
) -> None:
    """Fetch one Gem company, upsert jobs, advance lifecycle.
    ...
    conn = await asyncio.to_thread(
        ...
        application_name="task_fetch_gem",
    ...
    try:
        try:
            async def _work() -> None:
                ...
                # No verifier registered for Gem today → legacy
            ...
            logger.error(
                "fetch_gem_company failed for %s: %s",
        ...
        try:
            ...
            try:
                fallback_conn = await asyncio.to_thread(
                    db.get_connection,
                    settings.database_url,
                    application_name="task_fetch_gem_fallback",
                    statement_timeout_ms=60_000,
                )
                try:
