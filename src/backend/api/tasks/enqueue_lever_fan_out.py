"""Procrastinate periodic task: fan out per-company Lever fetches.
...
For each enabled Lever company, defers one ``fetch_lever_company``
...
from .fetch_lever_company import fetch_lever_company
...
async def enqueue_lever_fan_out(timestamp: int) -> int:
    """Defer one ``fetch_lever_company`` per enabled Lever company.
    ...
    if not companies:
        logger.info(
            "enqueue_lever_fan_out tick %d: no enabled lever companies",
            timestamp,
        )
        return 0
    ...
    for c in companies:
        ...
        try:
            ...
            logger.info(
                "fetch_lever_company already enqueued for %s; "
                "skipping this tick",
                company_id,
            )
            ...
            # in fetch_lever_company.py.
            failed += 1
            logger.exception(
                "Failed to defer fetch_lever_company for %s; "
                "continuing with remaining companies",
                company_id,
            )

    logger.info(
        "enqueue_lever_fan_out tick %d: deferred %d / %d companies "
        "(failed=%d)",
