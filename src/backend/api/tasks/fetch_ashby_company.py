"""Procrastinate task: fetch a single Ashby company's job board.

Per-company unit of work for the ashby fan-out cron (see Unit 5).
Fetches the live Ashby Job Board API for one company, upserts rows
...
from ..services.ashby_client import SOURCE_ID, fetch_jobs, transform_to_job_listings
...
async def fetch_ashby_company(
    company_id: str,
    board_token: str,
) -> None:
    """Fetch one Ashby company, upsert jobs, advance lifecycle.
    ...
    conn = await asyncio.to_thread(
        ...
        application_name="task_fetch_ashby",
    ...
    try:
        try:
            async def _work() -> None:
                ...
                # No verifier registered for Ashby today → falls through to
                # ``unknown_policy_for_unverified="close"`` (default), which
                # preserves legacy close-on-threshold behavior. Adding an
                # ``ashby_client.verify_url_alive`` later auto-flips this to
                # ``unknown_policy="skip"`` via the registry.
            ...
            logger.error(
                "fetch_ashby_company failed for %s: %s",
        ...
        try:
            ...
            try:
                fallback_conn = await asyncio.to_thread(
                    db.get_connection,
                    settings.database_url,
                    application_name="task_fetch_ashby_fallback",
                    statement_timeout_ms=60_000,
                )
                try:
