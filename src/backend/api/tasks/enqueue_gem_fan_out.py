"""Procrastinate periodic task: fan out per-company Gem fetches.
...
For each enabled Gem company, defers one ``fetch_gem_company`` job
with a per-company queueing lock so:
...
from .fetch_gem_company import fetch_gem_company
...
async def enqueue_gem_fan_out(timestamp: int) -> int:
    """Defer one ``fetch_gem_company`` per enabled Gem company.
    ...
    if not companies:
        logger.info(
            "enqueue_gem_fan_out tick %d: no enabled gem companies",
            timestamp,
        )
        return 0
    ...
    for c in companies:
        ...
        try:
            ...
            logger.info(
                "fetch_gem_company already enqueued for %s; "
                "skipping this tick",
                company_id,
            )
            ...
            # in fetch_gem_company.py.
            failed += 1
            logger.exception(
                "Failed to defer fetch_gem_company for %s; "
                "continuing with remaining companies",
                company_id,
            )

    logger.info(
        "enqueue_gem_fan_out tick %d: deferred %d / %d companies "
        "(failed=%d)",
