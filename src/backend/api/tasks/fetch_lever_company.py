"""Procrastinate task: fetch a single Lever company's postings.

Per-company unit of work for the lever fan-out cron (see Unit 5).
Fetches the live Lever Postings API for one company, upserts rows
...
async def fetch_lever_company(
    ...
    """Fetch one Lever company, upsert jobs, advance lifecycle.
    ...
    conn = await asyncio.to_thread(
        ...
        application_name="task_fetch_lever",
    ...
    try:
        try:
            ...
            logger.error(
                "fetch_lever_company failed for %s: %s",
        ...
        try:
            ...
            try:
                fallback_conn = await asyncio.to_thread(
                    db.get_connection,
                    settings.database_url,
                    application_name="task_fetch_lever_fallback",
                    statement_timeout_ms=60_000,
                )
                try:
