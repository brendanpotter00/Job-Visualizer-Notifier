    ...
    try:
        try:
            async def _work() -> None:
                ...
                # No verifier registered for Greenhouse today → legacy
                # close-on-threshold behavior preserved via the helper's
                # ``unknown_policy_for_unverified="close"`` default. Register
                # a ``greenhouse_client.verify_url_alive`` to flip into
                # fail-safe mode.
