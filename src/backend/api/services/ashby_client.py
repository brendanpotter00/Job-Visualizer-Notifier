"""Pure Ashby Job Board API client + transformer.

Two functions, both queue-agnostic:

- ``fetch_jobs(board_token, http)``: GETs Ashby's public Job Board API
  and returns the raw ``jobs`` array. No DB, no transformation.
- ``transform_to_job_listings(company_id, raw_jobs)``: maps each raw
  Ashby job dict to a :class:`scripts.shared.models.JobListing`
  ready for ``upsert_jobs_batch``.

The id format is the raw Ashby job id as a string (e.g. a UUID like
``"4e1c7a5a-5b0a-4e6f-9a7c-1ad3c4f7b6e9"``). Cross-source uniqueness
lives in the database schema via the composite ``(source_id, id)``
primary key on ``job_listings`` — Ashby rows use
``source_id = 'ashby_api'``. Ashby raw ids are UUIDs and globally
unique across the Ashby Job Board platform.

Output shape note: the ``details`` JSONB column is populated with keys that
the existing frontend ``backendScraperTransformer.ts`` reads
(``experience_level``, ``is_remote_eligible``). Ashby's public Job Board
API doesn't expose an experience level structurally, so we always emit
``experience_level`` as ``None``. ``is_remote_eligible`` is coerced from
``raw.isRemote`` via ``bool(...)`` so truthy/falsy/missing source values
all map to a clean boolean.
"""

from __future__ import annotations

import logging
from datetime import datetime, timezone
from typing import Any, Optional

import httpx

from scripts.shared.constants import SourceId
from scripts.shared.models import JobListing
from scripts.shared.utils import get_iso_timestamp

logger = logging.getLogger(__name__)

ASHBY_BASE_URL = "https://api.ashbyhq.com/posting-api/job-board"
DEFAULT_TIMEOUT_SECONDS = 30.0
SOURCE_ID = SourceId.ASHBY


async def fetch_jobs(board_token: str, http: httpx.AsyncClient) -> list[dict]:
    """Fetch all open jobs for an Ashby board.

    GETs ``{ASHBY_BASE_URL}/{board_token}?includeCompensation=true``. The
    ``includeCompensation=true`` param asks Ashby to include each job's
    compensation tier summary so we can populate ``details.compensation_summary``
    without a second round-trip.

    Raises ``httpx.HTTPStatusError`` on non-2xx and ``ValueError`` if the
    response JSON is missing the ``jobs`` key. The caller (Unit 4) treats
    both as a failed run and lets Procrastinate retry.

    The timeout is enforced **per call** so a long-lived shared
    ``httpx.AsyncClient`` (preferred at the caller layer) keeps its default
    timeout for other use. 30s is the PLAN-mandated value.
    """
    url = f"{ASHBY_BASE_URL}/{board_token}"
    logger.info("Fetching Ashby jobs for board %s", board_token)
    response = await http.get(
        url,
        params={"includeCompensation": "true"},
        timeout=DEFAULT_TIMEOUT_SECONDS,
    )
    response.raise_for_status()
    payload = response.json()
    if not isinstance(payload, dict) or "jobs" not in payload:
        raise ValueError(
            f"Ashby response for {board_token!r} missing 'jobs' key: "
            f"got keys {sorted(payload) if isinstance(payload, dict) else type(payload).__name__}"
        )
    jobs = payload["jobs"]
    if not isinstance(jobs, list):
        raise ValueError(
            f"Ashby 'jobs' for {board_token!r} is not a list: "
            f"got {type(jobs).__name__}"
        )
    logger.info("Ashby returned %d jobs for %s", len(jobs), board_token)
    return jobs


def transform_to_job_listings(
    company_id: str,
    raw_jobs: list[dict],
) -> list[JobListing]:
    """Map a list of Ashby job dicts to ``JobListing`` rows.

    See module docstring for the id format and ``details`` shape contracts.
    """
    now = get_iso_timestamp()
    return [_transform_one(company_id, raw, now) for raw in raw_jobs]


def _transform_one(
    company_id: str,
    raw: dict[str, Any],
    now: str,
) -> JobListing:
    """Transform a single Ashby job dict to a ``JobListing``."""
    raw_id = raw.get("id")
    if raw_id is None:
        raise ValueError(f"Ashby job missing 'id': {raw!r}")
    # Ashby ids are UUIDs (strings), but cast defensively in case a future
    # response shape ever ships a non-string id.
    job_id = str(raw_id)

    title = raw.get("title") or ""
    job_url = raw.get("jobUrl") or ""
    # Ashby's ``location`` is already a flat string (unlike Greenhouse's
    # ``{"name": ...}`` object), so it passes through directly.
    location = raw.get("location")

    secondary_locations = [
        s.get("location")
        for s in (raw.get("secondaryLocations") or [])
        if isinstance(s, dict) and s.get("location")
    ]

    posted_on_raw = raw.get("publishedAt")
    posted_on = _normalize_iso8601(posted_on_raw) if posted_on_raw else None
    if posted_on_raw and posted_on is None:
        # Per feedback_correctness_over_dont_crash: don't pass through a corrupt
        # timestamp string. Surface as a clean missing value (None) and log so the
        # data quality issue is visible in stderr (Railway @level:error).
        # ERROR (not WARNING): the comment block above promises stderr routing,
        # and Railway derives @level from Python log level.
        logger.error(
            "Ashby data quality issue: job %s for company %s had "
            "unparseable posted_on=%r; storing as NULL",
            raw_id,
            company_id,
            posted_on_raw,
        )

    details = {
        "department": raw.get("department"),
        "team": raw.get("team"),
        "secondary_locations": secondary_locations,
        "employment_type": raw.get("employmentType"),
        "is_remote_eligible": bool(raw.get("isRemote")),
        "compensation_summary": (raw.get("compensation") or {}).get(
            "compensationTierSummary"
        ),
        "published_at": raw.get("publishedAt"),
        "description_html": raw.get("descriptionHtml"),
        "experience_level": None,
    }

    return JobListing(
        id=job_id,
        title=title,
        company=company_id,
        location=location,
        url=job_url,
        source_id=SOURCE_ID,
        details=details,
        posted_on=posted_on,
        created_at=now,
        first_seen_at=now,
        last_seen_at=now,
        consecutive_misses=0,
        details_scraped=True,
        status="OPEN",
        has_matched=False,
        ai_metadata={},
        closed_on=None,
    )


def _normalize_iso8601(value: str) -> Optional[str]:
    """Parse an ISO 8601 string and re-emit as UTC ISO 8601.

    Returns ``None`` if parsing fails. The caller logs and stores ``None`` so
    a corrupt source string never silently makes it into ``job_listings``
    (per feedback_correctness_over_dont_crash). The row itself is preserved.
    """
    try:
        dt = datetime.fromisoformat(value)
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        return dt.astimezone(timezone.utc).isoformat()
    except (TypeError, ValueError):
        return None
