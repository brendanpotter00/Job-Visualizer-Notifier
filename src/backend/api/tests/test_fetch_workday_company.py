...
class TestFetchWorkdayCompany:
    ...
    async def test_partial_scrape_trips_guard_and_skips_destructive_writes(
        ...
        *called* separately in each of the six leaf tasks, so a divergence
        ...
        for i in range(100):
            _seed_job(db_conn, f"JR-SEED-{i}", company)

        postings = [_make_raw_workday_posting(f"JR-FRESH-{i}") for i in range(73)]
        # Workday paginates; hand the whole truncated page over once and
        # then return an empty page so the client's exit condition fires
        # regardless of how it advances its offset.
        pages = {"served": False}

        def handler(request: httpx.Request) -> httpx.Response:
            if pages["served"]:
                return httpx.Response(200, json=_make_workday_response([], total=73))
            pages["served"] = True
            return httpx.Response(200, json=_make_workday_response(postings, total=73))
