...
_default_probe = _discover._default_probe
...
def _measured_probe(case: dict) -> Callable[[str], tuple[int, str]]:
    ...
    tens of kilobytes, and only SIX things about it decide anything — the status, the
    length after script/style stripping, whether the job's own title is on it, whether
    the OTHER jobs' titles are (a related-jobs block, or a listing page served for every
    URL), and the two titles the page DECLARES: ``og:title`` and ``<title>``. The first
    four came from a real GET on 2026-08-29, the last two from a real GET on 2026-08-30
    (``titles_measured_at``).

    The declared pair is not decoration — it is the whole of what the 2026-08-30 prover
    change reads, and the corpus is where it has to keep saying no. Measured: Goldman's
    dead ``{roleId}`` declares *"Careers | Goldman Sachs"* on all three jobs, Kakao
    *"카카오 영입"* on all three, Walmart nothing at all. SpaceX is the adversarial
    control in the other direction: three genuinely different Greenhouse job pages that
    all declare ``og:title = "Accountant, Revenue"`` (and one whose ``<title>`` is the
    literal string ``page_title``), which is why equal declared titles are never a veto.
    ...
    def probe(url: str) -> tuple[int, str]:
        ...
        # The declared titles ride in a <head> the stripper deletes, exactly as they do
        # on the real page — an og:title lives in an ATTRIBUTE, so it contributes
        # nothing to `chars` and everything to `_declared_title`.
        head = ""
        if page.get("og_title"):
            head += f'<meta property="og:title" content="{page["og_title"]}">'
        if page.get("title_tag"):
            head += f'<title>{page["title_tag"]}</title>'
        head = f"<head>{head}</head>" if head else ""
        return page["status"], head + body + "x" * max(0, page["chars"] - len(body))
...
def test_job_link_corpus(name: str) -> None:
    ...
    proof = _prove_job_link(records, field_map, base_url, _measured_probe(case))
    assert proof.proved is case["proves"], f"{case['why']} (probe said: {proof.why})"
...
def test_the_three_dead_boards_are_all_refused() -> None:
    ...
    **AND EACH ONE HAS TO SURVIVE THE 2026-08-30 RELAXATION**, which is why this test
    now also asserts the verdict is a hard NO rather than a "we could not look":
    ``_resolve_job_link`` keeps a BLOCKED candidate ahead of the listing-page fallback,
    so a dead template that slid from *wrong* to *unproven* would start shipping again.
    Measured the same day, none of the three can: Jane Street 404s (the one status that
    is real evidence), Goldman declares *"Careers | Goldman Sachs"* on all three jobs,
    Walmart declares nothing at all and serves 1,606 identical chars.
    """
    two_hundreds = set()
    for name in ("janestreet", "goldman_pre_repair", "walmart"):
        case = _load(_JOB_LINKS / f"{name}.json")
        assert is_published_url_spec(case["records"], case["field_map"]["url"]) is False
        proof = _prove_job_link(
            case["records"], case["field_map"],
            "https://" + case["origin_url"].split("/")[2], _measured_probe(case),
        )
        assert proof.proved is False, f"{name}: {proof.why}"
        assert proof.blocked is False, (
            f"{name}: this board TOLD us the link is dead — calling that 'we could not "
            f"look' would put it back in front of the listing-page fallback. "
            f"Got: {proof.why}"
        )
        if name != "janestreet":
...
def test_probing_a_published_link_would_reject_a_working_board() -> None:
    """WHY BRANCH 1 NEVER FETCHES — **and the one board that still proves it**.

    This test used to name two: Atlassian and Roblox. The 2026-08-30 prover change
    settled Roblox, and its docstring predicted exactly that (*"if this test starts
    failing because the probe passes, the branch is still right — it just stopped being
    free"*). Roblox's page carries a related-jobs block, so every page holds the other
    jobs' titles and the pages differ by 0.8% — both halves of the OLD proof said no.
    Its ``og:title`` is that job's own title and the two differ, so the new proof
    agrees with reality. ``roblox.json`` now records ``proves: true``.

    **Atlassian does not move, and it is the whole argument on its own.** Its iCIMS
    page renders the job in an **iframe**: three different jobs serve 18,076 chars each,
    none carries its own title, and — measured the same day — the document declares no
    ``og:title`` and no ``<title>`` either. Over plain HTTP it is indistinguishable
    from Goldman's dead shell. Probing published links would still cost this board its
    links to catch nothing, and Atlassian is the board the brief names as
    must-not-regress.

    The proof is not asked to settle it: the board authored the path, so there is
    nothing of OURS to prove.
    """
    ...
    assert not any(p["og_title"] or p["title_tag"] for p in atlassian["pages"].values()), (
        "the iframe shell declares no title at all — that is why the 2026-08-30 title "
        "rule cannot rescue it, and why branch 1 must keep fetching nothing"
    )
    assert is_published_url_spec(atlassian["records"], atlassian["field_map"]["url"]) is True
    assert atlassian["proves"] is False
    assert _prove_job_link(
        atlassian["records"], atlassian["field_map"],
        "https://" + atlassian["origin_url"].split("/")[2], _measured_probe(atlassian),
    ).proved is False

    roblox = _load(_JOB_LINKS / "roblox.json")
    assert all(p["carries_own_title"] and p["carries_other_titles"]
               for p in roblox["pages"].values())
    assert roblox["proves"] is True, (
        "Roblox's published link is CORRECT and the prover now says so — if this "
        "regresses, the declared-title rule stopped working"
    )
...
def test_two_jobs_that_render_the_same_url_are_not_a_comparison() -> None:
    ...
    assert proof.proved is False and "distinct link" in proof.why
    assert proof.blocked is False        # nothing was fetched, so nothing blocked us
    assert fetched == []
...
def test_a_title_hiding_in_a_script_bundle_is_not_the_page() -> None:
    ...
    assert proof.proved is False and "same page" in proof.why
...
def test_a_probe_that_cannot_reach_the_board_proves_nothing() -> None:
    ...
    assert proof.proved is False and "no answer" in proof.why
    assert proof.blocked is True, (
        "a probe that never reached the board has learned nothing ABOUT the board — "
        "reporting it as a disproof is the IBM defect"
    )
...
# ==========================================================================
# DEFECT E — the prover cost ten correct recipes to catch three wrong ones
# ==========================================================================
#
# Measured across 27 boards (PATH-TO-90-PERCENT.md §3) and re-measured live on
# 2026-08-30. ``_prove_job_link`` rejected TEN correct job-link templates and caught
# THREE wrong ones. Every one of the ten failed for a reason that was about our CLIENT,
# not about the board:
#
#   * it never followed a cross-host redirect     -> SpaceX, Databricks reported HTTP 0
#   * it always sent the replay's Chrome UA       -> metacareers.com answered 400
#   * it read a WAF's silence as a denial         -> IBM "same page, 0 vs 0 chars"
#   * it could not see an og:title                -> JPMorgan, Micron, Oracle, Meta
#   * its 200-char floor outranked its own 2% bar -> YC/Raindrop, short by 48 chars
#
# Each fix is pinned separately, and each has a companion assertion for the board it
# must STILL refuse — the corpus above (Goldman, Walmart, Jane Street, Kakao, and
# Nintendo's embed) is what the prover exists for.


def _probe_client(handler, captured: list[httpx.Request]):
    """A real ``httpx.Client`` over a MockTransport, so ``_default_probe`` runs its
    actual streaming/retry code with no socket and no DNS."""
    def _record(request: httpx.Request) -> httpx.Response:
        captured.append(request)
        return handler(request)
    return lambda **kwargs: httpx.Client(
        transport=httpx.MockTransport(_record),
        headers={"User-Agent": "Mozilla/5.0 (Macintosh) Chrome/120.0.0.0 Safari/537.36"},
    )


def test_the_probe_retries_once_without_the_browser_user_agent(monkeypatch) -> None:
    """META. ``metacareers.com/jobs/<id>/`` answers **HTTP 400** to the replay's Chrome
    User-Agent and **200** to anything else — measured live 2026-08-30 on two real job
    ids, four UA variants each. The 400 was reported as "this link is not usable" and a
    correct recipe was thrown away.

    The browser UA stays FIRST because dropping it unconditionally costs other boards:
    with no UA header at all ``higher.gs.com`` and ``janestreet.com`` both answer 403,
    and with any non-browser UA ``careers.roblox.com`` tarpits to a read timeout
    (reproduced three times each). One retry, only when the board answered and refused.
    """
    seen: list[httpx.Request] = []

    def handler(request: httpx.Request) -> httpx.Response:
        if "Chrome" in request.headers.get("user-agent", ""):
            return httpx.Response(400, text="<html><title>Error</title></html>")
        return httpx.Response(200, text="<html><title>Software Engineer</title></html>")

    monkeypatch.setattr(_discover, "guarded_sync_client", _probe_client(handler, seen))
    status, body = _default_probe("https://www.metacareers.com/jobs/1/")

    assert (status, "Software Engineer" in body) == (200, True)
    assert len(seen) == 2, "exactly one retry, never a loop"
    assert "Chrome" in seen[0].headers["user-agent"]
    assert seen[1].headers["user-agent"] == _discover._PROBE_USER_AGENT


@pytest.mark.parametrize("status", [404, 410, 200, 500, 503])
def test_the_probe_does_not_retry_when_the_answer_is_about_the_url(
    status: int, monkeypatch
) -> None:
    """The retry is for "the board refused OUR CLIENT", not "the board answered".

    404/410 are the board answering the question we asked, 200 is an answer, and a 5xx
    is the board being broken — re-asking under a different name buys nothing and
    doubles the wall clock on the worst-case board. The budget matters: the whole
    job-link ladder shares a 75 s deadline inside a 240 s discovery task.
    """
    seen: list[httpx.Request] = []
    monkeypatch.setattr(
        _discover, "guarded_sync_client",
        _probe_client(lambda r: httpx.Response(status, text="body"), seen),
    )
    assert _default_probe("https://b.example/jobs/1")[0] == status
    assert len(seen) == 1


def test_a_probe_timeout_is_never_retried(monkeypatch) -> None:
    """THE WALL-CLOCK GUARANTEE. A timeout costs ``_LINK_PROBE_TIMEOUT_S``; retrying one
    would double the ladder's worst case (8 fetches x 10 s) into 160 s and put it past
    the deadline it is budgeted under. A refusal answers in milliseconds; a timeout does
    not, so only the refusal is retried.
    """
    seen: list[httpx.Request] = []
    ...
    monkeypatch.setattr(_discover, "guarded_sync_client", _probe_client(handler, seen))
    assert _default_probe("https://b.example/jobs/1") == (0, "")
    assert len(seen) == 1


def test_the_probe_asks_for_a_client_that_follows_cross_host_redirects(
    monkeypatch,
) -> None:
    """SPACEX and DATABRICKS. ``boards.greenhouse.io/spacex/jobs/8663938002`` 301s to
    ``job-boards.greenhouse.io`` and ``databricks.com`` 301s to ``www.databricks.com``
    — measured 2026-08-30, both plain redirects to the same company's own board, both
    reported by the host-pinned probe as **HTTP 0**, which reads as "unreachable".

    Every hop is still validated and IP-pinned; only the same-host pin is dropped, and
    only here. The SSRF half is proved in ``test_guarded_client.py``; what this asserts
    is that the probe actually asks for it.
    """
    ...
    monkeypatch.setattr(_discover, "guarded_sync_client", _client)
    _default_probe("https://boards.greenhouse.io/spacex/jobs/8663938002")
    assert asked == {"allow_cross_host": True}


# --- what the page SAYS it is -------------------------------------------------

@pytest.mark.parametrize("body,expected", [
    # JPMorgan's Oracle Fusion shell: 30 chars of text, a generic <title>, and the job's
    # real name in og:title. og WINS, and that is the whole fix.
    ('<html><head><title>JPMC Candidate Experience page</title>'
     '<meta property="og:title" content="AI Lead Security Engineer"/></head>'
     '<body>Loading</body></html>', "ai lead security engineer"),
    # Micron's Workday page: EMPTY <title>, og:title carries the job.
    ('<html><head><title></title>'
     '<meta name="title" property="og:title" content="TECHNOLOGIST - FAC UPW"></head>'
     '<body></body></html>', "technologist - fac upw"),
    # No og:title anywhere -> the <title> tag is the fallback (YC/Raindrop's shape).
    ("<html><head><title>Product Engineer at Raindrop</title></head></html>",
     "product engineer at raindrop"),
    # twitter:title counts too, and single quotes are a legal attribute delimiter.
    ("<html><head><meta name='twitter:title' content='Compiler Engineer'></head></html>",
     "compiler engineer"),
    # Walmart and Atlassian: the page declares nothing. Not a failure — the caller
    # falls through to comparing the pages themselves.
    ("<html><head></head><body>careers</body></html>", ""),
    ('<html><head><meta property="og:title" content=""><title></title></head></html>', ""),
])
def test_the_declared_title_is_og_first_then_the_title_tag(
    body: str, expected: str
) -> None:
    assert _declared_title(body) == expected


def test_an_og_title_is_invisible_to_the_page_text_it_is_compared_against() -> None:
    """WHY THIS IS A SEPARATE SIGNAL AND NOT MORE PAGE TEXT. ``_page_text`` strips tags,
    which deletes every ATTRIBUTE with them — so the one server-delivered per-job fact
    on a client-rendered board contributes exactly zero characters to the length
    comparison. That is the structural reason five correct boards read as "same page".
    """
    shell = ('<html><head><meta property="og:title" content="Staff Compiler Engineer">'
             "</head><body>Loading</body></html>")
    assert _page_text(shell) == "loading"
    assert _declared_title(shell) == "staff compiler engineer"


def test_two_pages_that_declare_different_titles_are_proved() -> None:
    """JPMORGAN, live 2026-08-30: ``…/job/210775811`` and ``…/job/210776728`` both serve
    a 30-char shell titled *"JPMC Candidate Experience page"* — identical to the
    stripper — with og:title *"AI Lead Security Engineer"* vs *"Credit Card Customer
    Service Account Specialist I"*. The old prover said "same page, 30 vs 30 chars".
    """
    body = (
        '<html><head><title>JPMC Candidate Experience page</title>'
    ...
    pages = {
        "https://b.example/job/210775811": body.format(og="AI Lead Security Engineer"),
        "https://b.example/job/210776728": body.format(
            og="Credit Card Customer Service Account Specialist I"),
    }
    records = [{"id": "210775811", "title": "AI Lead Security Engineer"},
               {"id": "210776728",
                "title": "Credit Card Customer Service Account Specialist I"}]
    field_map = {"id": "id", "title": "title", "url": "https://b.example/job/{id}"}
    assert _pages_differ(*(_page_text(p) for p in pages.values())) is False
    assert _prove_job_link(
        records, field_map, "https://b.example", lambda url: (200, pages[url])
    ).proved is True


def test_an_identical_declared_title_is_never_a_veto() -> None:
    """SPACEX IS THE CONTROL, and it is in the corpus. Three genuinely different
    Greenhouse job pages all declare ``og:title = "Accountant, Revenue"`` (one's
    ``<title>`` is the literal string ``page_title``) — measured 2026-08-30. If equal
    declared titles meant "no", the title rule would break a board it was supposed to
    leave alone. Equal means NO EVIDENCE; the length comparison still decides.
    """
    head = '<head><meta property="og:title" content="Accountant, Revenue"></head>'
    pages = {
        "https://b.example/jobs/1": f"<html>{head}<body>{'x' * 12473}</body></html>",
        "https://b.example/jobs/2": f"<html>{head}<body>{'x' * 11996}</body></html>",
    }
    records = [{"id": 1, "title": "Accountant, Revenue"},
               {"id": 2, "title": "Accountant, Revenue"}]
    assert _prove_job_link(
        records, {"id": "id", "title": "title", "url": "https://b.example/jobs/{id}"},
        "https://b.example", lambda url: (200, pages[url]),
    ).proved is True


@pytest.mark.parametrize("declared,chars", [
    # Nintendo's Greenhouse embed — the LISTING page under every gh_jid (AC-20).
    ('<meta property="og:title" content="Careers at Nintendo - Join Our Team">'
     "<title>Careers at Nintendo - Join Our Team</title>", 842),
    ("<title>Careers | Goldman Sachs</title>", 23),   # the dead {roleId}
    ("", 1606),                                        # Walmart declares nothing
])
def test_a_generic_declared_title_on_every_job_is_still_refused(
    declared: str, chars: int
) -> None:
    """THE THREE THE TITLE RULE MUST NOT RESCUE, all measured live 2026-08-30 and all
    serving one title (or none) to every job."""
    body = f"<html><head>{declared}</head><body>{'x' * chars}</body></html>"
    records = [{"id": 1, "title": "Brand Ambassador Part-Time"},
               {"id": 2, "title": "Software Engineer II"}]
    proof = _prove_job_link(
        records, {"id": "id", "title": "title", "url": "https://b.example/jobs/{id}"},
        "https://b.example", lambda url: (200, body),
    )
    assert (proof.proved, proof.blocked) == (False, False)


# --- silence is not a denial --------------------------------------------------

@pytest.mark.parametrize("status,body,blocked", [
    (202, "", True),        # IBM: AWS WAF, 202 and zero bytes on every job page
    (403, "denied", True),  # a bot filter
    (429, "slow down", True),
    (400, "<html><title>Error</title></html>", True),
    (0, "", True),          # a timeout, a DNS failure, a guard refusal
    (404, "Page not found", False),   # Jane Street: the board ANSWERED
    (410, "gone", False),
])
def test_a_board_that_would_not_answer_is_unproven_not_disproved(
    status: int, body: str, blocked: bool
) -> None:
    """IBM. AWS WAF answers ``careers.ibm.com/careers/JobDetail?jobId=<id>`` with
    **202 and an empty body** on every job page — measured 2026-08-30 under four
    different User-Agents. The old prover compared the two empty bodies, found them
    equal, and reported *"two different jobs served the same page (0 vs 0 chars)"*: a
    positive claim about the board built out of bytes the board never sent.

    404 and 410 stay hard NOs. They are the board answering the question we asked, and
    they are the only status that is real evidence a template is wrong — Jane Street's
    ``/jobs/{id}`` is exactly this and must never become "unproven".
    """
    records = [{"id": i, "title": f"Engineer number {i}"} for i in range(3)]
    proof = _prove_job_link(
        records, {"id": "id", "title": "title", "url": "https://b.example/jobs/{id}"},
        "https://b.example", lambda url: (status, body),
    )
    assert (proof.proved, proof.blocked) == (False, blocked)


def test_an_spa_shell_is_a_real_answer_and_stays_a_hard_no() -> None:
    """THE LINE BETWEEN BLOCKED AND WRONG, drawn where Goldman is on the right side.

    "Blocked" is an EMPTY body, not "a body we could read nothing out of". A
    ``<script>``-only shell is thousands of bytes of genuine answer, and Goldman's dead
    ``{roleId}`` differed from its working link by 23 of them. Widening blocked to mean
    "empty after stripping" would move that board from disproved to unproven — and
    ``_resolve_job_link`` KEEPS an unproven candidate, so the 404 this whole rule exists
    to stop would ship again.
    """
    shell = ('<html><head></head><body><script>window.__DATA__={"jobs":[]}</script>'
             "</body></html>")
    records = [{"id": 1, "title": "Distributed Systems Engineer"},
               {"id": 2, "title": "Compiler Engineer, Runtime"}]
    proof = _prove_job_link(
        records, {"id": "id", "title": "title", "url": "https://b.example/jobs/{id}"},
        "https://b.example", lambda url: (200, shell),
    )
    assert _page_text(shell) == "" and _declared_title(shell) == ""
    assert (proof.proved, proof.blocked) == (False, False)


# --- the floor that outranked its own fraction --------------------------------

def test_the_absolute_delta_floor_no_longer_overrules_the_fraction() -> None:
    """YC/RAINDROP. 7,088 vs 6,936 chars — a 152-char difference against a 2% bar of
    141. The fraction said "different"; the flat 200-char floor overruled it and a
    correct recipe was rejected 48 chars short.

    120 is where the corpus puts the floor, not a taste. Across the 13 boards in
    ``fixtures/job_links`` and the 10 re-measured on 2026-08-30, every WRONG template
    serves byte-identical pages (delta 0) and the smallest difference any CORRECT link
    produced is Roblox's 50 chars. Every threshold in (0, 50) separates the corpus, so
    120 keeps a wide margin over both.
    """
    assert _discover._MIN_PAGE_DELTA_CHARS == 120
    assert _pages_differ("x" * 7088, "x" * 6936) is True     # YC: was 48 chars short
    assert _pages_differ("x" * 6447, "x" * 6397) is False    # Roblox's 50: still noise
    assert _pages_differ("x" * 1606, "x" * 1606) is False    # Walmart: the wrong shape
    # ...and the fraction still bounds the absolute on a big page.
    assert _pages_differ("x" * 700_000, "x" * 700_300) is False
