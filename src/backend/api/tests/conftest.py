...
def _make_job(overrides: dict | None = None) -> dict:
    ...
    # endpoint reads these three denormalized columns, NOT details->..., to avoid
    ...
    # with details.department/experience_level/is_remote_eligible behaves like a
    # real upsert. Deriving matters more than it looks: a fixture that hand-set
    # `details` only is exactly how the missing `department` column stayed
    # invisible to the whole test suite while production served nothing.
    _details = base["details"]
    if isinstance(_details, str):
        _details = json.loads(_details)
    base.setdefault("department", _details.get("department"))
