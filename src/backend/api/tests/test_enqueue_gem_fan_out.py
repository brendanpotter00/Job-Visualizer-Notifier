"""Integration tests: enqueue_gem_fan_out periodic task body."""
...
from api.tasks.enqueue_gem_fan_out import enqueue_gem_fan_out
...
async def procrastinate_open(db_conn):
    ...
    try:
        ...
        try:
            ...
            # rows so each test sees a clean queue, and truncate the
            # `companies` table so each test seeds its own roster.
            cur = db_conn.cursor()
            cur.execute(
...
class TestEnqueueGemFanOut:
    async def test_defers_one_job_per_enabled_gem_company(
        self, procrastinate_open, db_conn
    ):
        enabled_ids = [f"co{i}" for i in range(3)]
        for cid in enabled_ids:
            _seed_company(db_conn, cid)

        _seed_company(db_conn, "disabled_co", enabled=False)
        _seed_company(db_conn, "ashby_co", ats="ashby")
        _seed_company(db_conn, "greenhouse_co", ats="greenhouse")

        deferred = await enqueue_gem_fan_out(timestamp=0)
        db_conn.rollback()
        ...
        expected_locks = {f"gem:{cid}" for cid in enabled_ids}
    ...
    async def test_already_enqueued_per_company_continues_loop(
        ...
        first = await enqueue_gem_fan_out(timestamp=0)
        db_conn.rollback()
        assert first == 3

        second = await enqueue_gem_fan_out(timestamp=1)
        db_conn.rollback()
        assert second == 0
    ...
    async def test_per_company_connector_error_does_not_abort_loop(
        ...
        catch in enqueue_gem_fan_out.
        """
        import psycopg2 as _psycopg2

        import api.tasks.enqueue_gem_fan_out as fan_out_mod

        ids = [f"co{i}" for i in range(5)]
        for cid in ids:
            _seed_company(db_conn, cid)

        real_configure = fan_out_mod.fetch_gem_company.configure
        call_count = {"n": 0}
    ...
    async def test_programmer_error_propagates(
        ...
        narrow `(ConnectorException, psycopg2.Error)` catch."""
        import api.tasks.enqueue_gem_fan_out as fan_out_mod
        ...
        with pytest.raises(AttributeError):
            await enqueue_gem_fan_out(timestamp=0)
        db_conn.rollback()

    async def test_no_enabled_companies_returns_zero(
        self, procrastinate_open, db_conn
    ):
        """Seed only disabled / non-gem companies — fan-out returns 0
        with no errors and no defers."""
