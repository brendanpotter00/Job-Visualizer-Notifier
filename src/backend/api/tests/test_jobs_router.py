...
def test_read_path_reflects_sidecar_not_stale_job_listings(client, db_conn):
    """Units 3+4: both /api/jobs and /api/jobs/{id} source last_seen_at /
    consecutive_misses from the job_freshness sidecar (via JOIN).

    Before Unit 4 this test diverged the two tables and asserted the read
    followed the sidecar. The contract migration (18fe9c20a8fd) dropped the
    job_listings copies, so there is nothing left to diverge *from* — the
    remaining guard is that a sidecar-only advance (the shape the Unit-2 write
    path produces) reaches the response, while the untouched job_listings
    timestamps stay where they were seeded.
    """
    _insert_job(db_conn, _make_job({
        "id": "sidecar-read-1", "company": "google", "source_id": SourceId.GOOGLE,
        "status": "OPEN", "first_seen_at": "2025-01-01T00:00:00Z",
        "last_seen_at": "2025-01-01T00:00:00Z",
    }))
    cur = db_conn.cursor()
    # Sidecar advances (as the Unit-2 write path would); job_listings is not
    # touched — it no longer carries these two columns at all.
    cur.execute(
    ...
    assert _parse_iso(listed["firstSeenAt"]) == _parse_iso("2025-01-01T00:00:00Z")
...
# -- List endpoint serves department (the Department filter's only data source) --


def test_list_endpoint_returns_department(client, db_conn):
    """`details.department` must be in the LIST response, not just the detail one.

    The frontend transformer maps `details.department` -> `Job.department`,
    `selectAvailableDepartments` uniques it, and `GraphFilters` renders the
    Department control only when that list is non-empty. Before the `department`
    column existed, `_LIST_COLUMNS` built `details` from two keys and the control
    was hidden on every company — while 20,670 of 32,022 open production rows had
    a real department sitting in the untouched `details` JSONB.
    """
    _insert_job(db_conn, _make_job({
        "id": "dept-list-test",
        "details": json.dumps({
    ...
    job = next(j for j in resp.json() if j["id"] == "dept-list-test")
    details = json.loads(job["details"])
    assert details["department"] == "Cloud Infrastructure"
    # Still trimmed — adding department must not drag the wide blob back in.
    assert "about_the_job" not in details


def test_list_endpoint_reads_department_from_the_column_not_the_jsonb(client, db_conn):
    """Load-bearing: the value comes from the denormalized `department` COLUMN.

    Reading `details->>'department'` in the projection would produce the same
    answer on this row and re-create the 2026-07-13 outage at prod scale — one
    JSONB key access detoasts the whole ~10 KB value per row. Seeding the column
    and the JSONB with DIFFERENT values is the only assertion that can tell the
    two implementations apart.
    """
    _insert_job(db_conn, _make_job({
        "id": "dept-source-test",
        "details": json.dumps({"department": "FROM-THE-JSONB"}),
        "department": "FROM-THE-COLUMN",
    }))

    resp = client.get("/api/jobs", params={"company": "google"})

    job = next(j for j in resp.json() if j["id"] == "dept-source-test")
    assert json.loads(job["details"])["department"] == "FROM-THE-COLUMN"


def test_list_endpoint_returns_null_department_for_a_board_without_one(client, db_conn):
    """Google/Apple/Microsoft publish no department. The key must be present and
    null rather than absent, so the transformer's `details.department` is a clean
    `undefined` and no empty option reaches the Department dropdown."""
    _insert_job(db_conn, _make_job({
        "id": "dept-null-test",
        "details": json.dumps({"experience_level": "Senior"}),
    }))

    resp = client.get("/api/jobs", params={"company": "google"})

    job = next(j for j in resp.json() if j["id"] == "dept-null-test")
    assert json.loads(job["details"])["department"] is None
