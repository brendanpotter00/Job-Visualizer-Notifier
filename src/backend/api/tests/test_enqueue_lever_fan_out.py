"""Integration tests: enqueue_lever_fan_out periodic task body."""
...
async def procrastinate_open(db_conn):
    ...
    try:
        ...
        try:
            ...
            # across cases. Wipe both lever task rows so each test sees a
...
class TestEnqueueLeverFanOut:
    async def test_defers_one_job_per_enabled_lever_company(
        ...
        expected_locks = {f"lever:{cid}" for cid in enabled_ids}
    ...
    async def test_already_enqueued_per_company_continues_loop(
        ...
        first = await enqueue_lever_fan_out(timestamp=0)
        db_conn.rollback()
        assert first == 3

        second = await enqueue_lever_fan_out(timestamp=1)
        db_conn.rollback()
        assert second == 0
    ...
    async def test_per_company_connector_error_does_not_abort_loop(
        ...
        catch in enqueue_lever_fan_out.

        Strategy: monkeypatch fetch_lever_company.configure(...).defer_async
        ...
        import api.tasks.enqueue_lever_fan_out as fan_out_mod

        ids = [f"co{i}" for i in range(5)]
        for cid in ids:
            _seed_company(db_conn, cid)

        real_configure = fan_out_mod.fetch_lever_company.configure
        call_count = {"n": 0}
        ...
        monkeypatch.setattr(
            fan_out_mod.fetch_lever_company,
            "configure",
    ...
    async def test_programmer_error_propagates(
        ...
        with pytest.raises(AttributeError):
            await enqueue_lever_fan_out(timestamp=0)
        db_conn.rollback()
