...
def test_trigger_scrape_returns_409_when_scrape_in_progress(client):
    ...
    # Use a fresh event loop rather than asyncio.get_event_loop(): once a
    # prior async test in the suite has run and closed its loop, the implicit
    # getter raises "There is no current event loop" on Python 3.10+, making
    # this test order-dependent. scraper_lock.locked() is loop-independent, so
    # acquiring on a dedicated loop is sufficient to simulate an in-progress
    # scrape for the endpoint's check.
...
# --- trigger-gem-fetch / trigger-gem-fan-out ---


def _gem_jobs(conn) -> list[dict]:
    cur = conn.cursor()
    cur.execute(
        "SELECT id, task_name, args, queueing_lock, status "
        "FROM procrastinate_jobs "
...
def _gem_fan_out_jobs(conn) -> list[dict]:
    cur = conn.cursor()
    cur.execute(
        "SELECT id, task_name, args, queueing_lock, status "
        "FROM procrastinate_jobs "
...
@pytest.mark.asyncio
async def test_trigger_gem_fetch_returns_202_and_enqueues_job(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "retool", ats="gem", board_token="retool")

    resp = client.post(
        "/api/jobs-qa/trigger-gem-fetch",
        params={"company_id": "retool"},
    )
    db_conn.rollback()

    assert resp.status_code == 202
    body = resp.json()
    assert body["company_id"] == "retool"
    assert body["already_enqueued"] is False
    assert "deferred" in body["message"]

    jobs = _gem_jobs(db_conn)
    assert len(jobs) == 1
    assert jobs[0]["queueing_lock"] == "gem:retool"
    assert jobs[0]["args"]["company_id"] == "retool"
    assert jobs[0]["args"]["board_token"] == "retool"


@pytest.mark.asyncio
async def test_trigger_gem_fetch_404_for_unknown_company(
    procrastinate_open, db_conn, client,
):
    resp = client.post(
        "/api/jobs-qa/trigger-gem-fetch",
    ...
    assert resp.status_code == 404
    assert "does_not_exist" in resp.json()["detail"]
    assert _gem_jobs(db_conn) == []


@pytest.mark.asyncio
async def test_trigger_gem_fetch_404_for_disabled_company(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "off", ats="gem", enabled=False)
    resp = client.post(
        "/api/jobs-qa/trigger-gem-fetch",
        params={"company_id": "off"},
    )
...
@pytest.mark.asyncio
async def test_trigger_gem_fetch_404_for_non_gem_ats(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "stripe", ats="greenhouse")
    resp = client.post(
...
@pytest.mark.asyncio
async def test_trigger_gem_fetch_second_call_reports_already_enqueued(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "retool", ats="gem")
    ...
    assert len(_gem_jobs(db_conn)) == 1


@pytest.mark.asyncio
async def test_trigger_gem_fan_out_returns_202_and_enqueues_task(
    procrastinate_open, db_conn, client,
):
    resp = client.post("/api/jobs-qa/trigger-gem-fan-out")
    db_conn.rollback()
    ...
    jobs = _gem_fan_out_jobs(db_conn)
    assert len(jobs) == 1
    assert jobs[0]["args"]["timestamp"] == 0


def test_trigger_gem_fetch_without_admin_returns_403(test_app, db_conn):
    from fastapi.testclient import TestClient
    from api.auth.dependencies import require_admin
    from .conftest import _insert_user, _make_user
...
def test_trigger_gem_fetch_without_auth_returns_401(test_app):
    """Pop BOTH require_admin and get_current_user overrides so the real
    auth chain runs without an Authorization header. Mirrors the Ashby
    sibling test."""
    from fastapi.testclient import TestClient
    from api.auth.dependencies import get_current_user, require_admin
...
def test_trigger_gem_fan_out_without_admin_returns_403(test_app, db_conn):
    from fastapi.testclient import TestClient
    from api.auth.dependencies import require_admin
    from .conftest import _insert_user, _make_user
    ...
    try:
        local_client = TestClient(test_app)
        resp = local_client.post("/api/jobs-qa/trigger-gem-fan-out")
        assert resp.status_code == 403
    finally:
        if saved_override is not None:
            test_app.dependency_overrides[require_admin] = saved_override
...
def test_trigger_gem_fan_out_without_auth_returns_401(test_app):
    """Mirrors test_trigger_gem_fetch_without_auth_returns_401 for the
    fan-out endpoint."""
    from fastapi.testclient import TestClient
    from api.auth.dependencies import get_current_user, require_admin
...
# --- trigger-lever-fetch / trigger-lever-fan-out ---


def _lever_jobs(conn) -> list[dict]:
    cur = conn.cursor()
    cur.execute(
        "SELECT id, task_name, args, queueing_lock, status "
        "FROM procrastinate_jobs "
...
def _lever_fan_out_jobs(conn) -> list[dict]:
    cur = conn.cursor()
    cur.execute(
        "SELECT id, task_name, args, queueing_lock, status "
        "FROM procrastinate_jobs "
...
@pytest.mark.asyncio
async def test_trigger_lever_fetch_returns_202_and_enqueues_job(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "palantir", ats="lever", board_token="palantir")

    resp = client.post(
        "/api/jobs-qa/trigger-lever-fetch",
        params={"company_id": "palantir"},
    )
    db_conn.rollback()

    assert resp.status_code == 202
    body = resp.json()
    assert body["company_id"] == "palantir"
    assert body["already_enqueued"] is False
    assert "deferred" in body["message"]

    jobs = _lever_jobs(db_conn)
    assert len(jobs) == 1
    assert jobs[0]["queueing_lock"] == "lever:palantir"
    assert jobs[0]["args"]["company_id"] == "palantir"
    assert jobs[0]["args"]["board_token"] == "palantir"


@pytest.mark.asyncio
async def test_trigger_lever_fetch_404_for_unknown_company(
    procrastinate_open, db_conn, client,
):
    resp = client.post(
        "/api/jobs-qa/trigger-lever-fetch",
    ...
    assert resp.status_code == 404
    assert "does_not_exist" in resp.json()["detail"]
    assert _lever_jobs(db_conn) == []


@pytest.mark.asyncio
async def test_trigger_lever_fetch_404_for_disabled_company(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "off", ats="lever", enabled=False)
    resp = client.post(
        "/api/jobs-qa/trigger-lever-fetch",
        params={"company_id": "off"},
    )
...
@pytest.mark.asyncio
async def test_trigger_lever_fetch_404_for_non_lever_ats(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "stripe", ats="greenhouse")
    resp = client.post(
...
@pytest.mark.asyncio
async def test_trigger_lever_fetch_second_call_reports_already_enqueued(
    procrastinate_open, db_conn, client,
):
    _seed_company(db_conn, "palantir", ats="lever")
    ...
    assert len(_lever_jobs(db_conn)) == 1


@pytest.mark.asyncio
async def test_trigger_lever_fan_out_returns_202_and_enqueues_task(
    procrastinate_open, db_conn, client,
):
    resp = client.post("/api/jobs-qa/trigger-lever-fan-out")
    db_conn.rollback()
    ...
    jobs = _lever_fan_out_jobs(db_conn)
    assert len(jobs) == 1
    assert jobs[0]["args"]["timestamp"] == 0


@pytest.mark.asyncio
async def test_trigger_lever_fetch_after_fan_out_defer_collapses_via_lock(
    procrastinate_open, db_conn, client,
):
    """PLAN invariant: a manual /trigger-lever-fetch issued AFTER the
    periodic fan-out has already deferred a per-company job for the same
    company collapses via the `lever:<company_id>` queueing lock.
    """
    from api.tasks.enqueue_lever_fan_out import enqueue_lever_fan_out

    _seed_company(db_conn, "palantir", ats="lever")
    db_conn.rollback()

    deferred = await enqueue_lever_fan_out(timestamp=0)
    db_conn.rollback()
    assert deferred == 1, (
        f"periodic fan-out should defer exactly 1 job for the seeded "
        f"company; got {deferred}"
    ...
    jobs = _lever_jobs(db_conn)
    assert len(jobs) == 1, (
        f"expected exactly 1 fetch_lever_company row after fan-out + "
        f"manual trigger collision; got {len(jobs)}. Locks: "
        f"{[j['queueing_lock'] for j in jobs]}"
    )
    assert jobs[0]["queueing_lock"] == "lever:palantir"
...
def test_trigger_lever_fetch_without_admin_returns_403(test_app, db_conn):
    from fastapi.testclient import TestClient
    from api.auth.dependencies import require_admin
    from .conftest import _insert_user, _make_user
...
def test_trigger_lever_fetch_without_auth_returns_401(test_app):
    from fastapi.testclient import TestClient
    from api.auth.dependencies import get_current_user, require_admin
...
def test_trigger_lever_fan_out_without_admin_returns_403(test_app, db_conn):
    from fastapi.testclient import TestClient
    from api.auth.dependencies import require_admin
    from .conftest import _insert_user, _make_user
    ...
    try:
        local_client = TestClient(test_app)
        resp = local_client.post("/api/jobs-qa/trigger-lever-fan-out")
        assert resp.status_code == 403
    finally:
        if saved_override is not None:
            test_app.dependency_overrides[require_admin] = saved_override
