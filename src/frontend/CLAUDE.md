...
- `components/layout/NavigationDrawer.tsx:137` — `react-hooks/set-state-in-effect`. Auto-opens the collapsible Admin sidebar section once `useCurrentUser` resolves to an admin profile. `isAdmin` flips from `false` → `true` asynchronously (after the `/api/users` fetch), so a `useState(isAdmin)` initial value would lock the section closed for the whole session. The effect mirrors the flag on the *true* edge only, so a manual collapse by the admin is not re-overridden on subsequent renders.
