...
- The timeline chart in `components/companies-page/JobPostingsChart/GraphSection.tsx` is collapsible (local `useState`, default expanded, not persisted); collapsing hides only the chart — the filters stay visible because they also drive the list
...
- `features/filters/slices/graphFiltersSlice.ts:72` — `@typescript-eslint/no-explicit-any`. `createFilterSlice` generates action creators via computed property names (`[set${CapitalizedName}TimeWindow]`), which TypeScript cannot infer through. The `as any` cast on `slice.actions` is the documented TS limitation (see https://github.com/reduxjs/redux-toolkit/issues/368). Types are still enforced at dispatch sites.
- `features/filters/slices/recentJobsFiltersSlice.ts:79` — `@typescript-eslint/no-explicit-any`. Same rationale as `graphFiltersSlice.ts`.
...
- `features/settings/subcategoryReveal.tsx:56` — `react-refresh/only-export-components`. Same rule, same reason, same shape as the entry above: the file exports the `SubcategoryRevealProvider` component alongside the `useSubcategoryRevealEnabled` hook its consumers read the flag through. The two belong together — the whole point of the module is that there is exactly ONE subscription to `GET /api/jobs/settings` in the app, and splitting the provider from its reader invites a second one.
