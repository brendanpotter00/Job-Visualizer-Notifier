...
/**
 * Greenhouse and Ashby boards now flow through the backend `/api/jobs` endpoint,
 * so they share the `backend-scraper` ATS type with the true custom scrapers
 * (Google, Apple, Microsoft). The `Company.sourceAts` field tags migrated
 * providers so the Why page can show them in their own column instead of
