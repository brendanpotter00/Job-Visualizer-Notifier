...
const MATRIX_GRID_SX = {
  display: 'grid',
  gridTemplateColumns: {
    xs: `repeat(${MATRIX_COLUMNS.xs}, minmax(0, 1fr))`,
    sm: `repeat(${MATRIX_COLUMNS.sm}, minmax(0, 1fr))`,
  },
  gap: 0,
} as const;
...
function MatrixCell({ feature, muted = false, spans }: MatrixCellProps) {
    ...
    <Box
      ...
      sx={{
        ...
        ...(spans &&
          (spans.xs || spans.sm) && {
            gridColumn: {
              xs: spans.xs ? `span ${MATRIX_COLUMNS.xs}` : 'span 1',
              sm: spans.sm ? `span ${MATRIX_COLUMNS.sm}` : 'span 1',
            },
          }),
      }}
    >
      ...
      <Box data-testid="feature-matrix-live" sx={MATRIX_GRID_SX}>
        {features.map((feature, i) => (
          <MatrixCell
            key={feature.id}
            feature={feature}
            spans={matrixCellSpans(features.length, i)}
          />
        ))}
      </Box>
        ...
        <Box sx={MATRIX_GRID_SX}>
          {comingSoon.map((feature, i) => (
            <MatrixCell
              ...
              feature={feature}
              muted
              spans={matrixCellSpans(comingSoon.length, i)}
            />
          ))}
        </Box>
      </Box>
