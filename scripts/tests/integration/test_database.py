...
class TestInitSchema:
    ...
    def test_init_schema_creates_indexes(self, in_memory_db):
        ...
        assert f"idx_{db.JOBS_TABLE}_status" in indexes
        assert f"idx_{db.JOBS_TABLE}_company" in indexes
        assert f"idx_{db.JOBS_TABLE}_last_seen" in indexes
...
class TestScrapeRun:
    ...
    def test_record_scrape_run(self, in_memory_db, sample_scrape_run):
        ...
        cursor = in_memory_db.cursor()
        cursor.execute(
            f"SELECT * FROM {db.RUNS_TABLE} WHERE run_id = %s",
