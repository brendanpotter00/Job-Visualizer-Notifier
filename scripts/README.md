...
- **Env-agnostic tables**: Table names (`job_listings`, `scrape_runs`, `users`, `user_enabled_companies`) are identical across every environment; test isolation is handled via per-worker Postgres schemas, not table suffixes.
