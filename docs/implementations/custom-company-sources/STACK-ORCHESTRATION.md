...
| P3 | `feat/e7-phase3-discovery` | P2 | — | Phase 3 — closed primitive vocabulary + stored HTTP scripts + one-time local-browser agent discovery (§6 Phase 3, §9) | 🔜 planning next |
| P4 | `feat/e7-phase4-browser-runtime` | P3 | — | Phase 4 — Browserbase runtime + execution-time SSRF/CDP host pinning (§6 Phase 4) | ⛔ **WAIT FOR ARTICLES (Brendan)** — do NOT plan/implement P4 until he sends the browser articles; see memory `pause-before-phase4-browser-for-articles`. Then Browserbase (free-tier key in `.env.local`). |
