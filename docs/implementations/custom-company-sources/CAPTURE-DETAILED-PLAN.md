...
**Base decision (needs your call):** the capture feature builds on the **#248-level surface** (the
recipe engine `recipe_runner`/`recipe_schema`/`recipe_rows`/`guarded_client` are new on #248; the gate,
storage, `url_guard`, add-flow are on #247). PR #258 is currently **docs-only off `main`**. Two clean options:

- **(A, recommended)** Land the validated E7 stack (#243→#247→#248) to `main` first, then rebase PR #258
  and stack the capture PRs on `main`. The recipe engine + gate + storage come along; the capture work
  retires the Stagehand half. Cleanest history; empties the tall unmerged stack.
- **(B)** Don't merge yet — rebase PR #258 onto `feat/e7-phase3-discovery` and stack the capture PRs on
  it (modify-in-place). Faster, but keeps everything unmerged.

**The stack (each builds on the previous):**
