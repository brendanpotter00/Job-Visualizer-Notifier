...
**Adding a Company:**
Edit `src/frontend/src/config/companies.ts` and add company config with ATS type (workday/gem/eightfold/backend-scraper). Greenhouse, Ashby, and Lever boards use `backend-scraper` — add the company id and `board_token` to the backend `companies` table (see `docs/implementations/greenhouseBackendMigration/PLAN.md`, `docs/implementations/ashbyBackendMigration/PLAN.md`, or `docs/implementations/leverBackendMigration/PLAN.md`) and add a `createBackendScraperCompany(id, name, 'https://boards.greenhouse.io/${id}', { sourceAts: 'greenhouse' })` entry to companies.ts (use `{ sourceAts: 'ashby' }` for Ashby boards or `{ sourceAts: 'lever' }` for Lever boards). Omitting `sourceAts` drops the company into "Custom Web Scrapers". System automatically uses correct client via factory pattern.
